...
for(var i = 0; i < 18; i++){
    print(limit + "--" + skip);
    db.getCollection('javaliLibraries_PENDENTE_JUNIT').find({'percentage': null}).sort({"index": 1}).limit(limit).skip(skip).forEach(function(val){
            ...
            db.getCollection('javaliLibraries_CALCULADO_JUNIT').insert(resultado);
    });
